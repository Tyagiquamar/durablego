...
func Start(ctx context.Context) (string, func(), error) {
	if url := os.Getenv("DURABLEGO_TEST_DATABASE_URL"); url != "" {
		if err := migrate(ctx, url); err != nil {
			return "", nil, fmt.Errorf("apply migrations to DURABLEGO_TEST_DATABASE_URL: %w", err)
		}
		return url, func() {}, nil
	}
	container, err := postgres.Run(ctx, image,
		postgres.WithDatabase("durablego"),
		postgres.WithUsername("durablego"),
		postgres.WithPassword("durablego"),
...
func migrate(ctx context.Context, url string) error {
	...
	defer pool.Close()
	var existing bool
	if err := pool.QueryRow(ctx, `SELECT to_regclass('public.workflow_executions') IS NOT NULL`).Scan(&existing); err != nil {
		return err
	}
	if existing {
		return nil
...
func Shared(t testing.TB) string {
	...
	if sharedHarness.url == "" {
		...
		if err != nil {
			t.Fatalf("PostgreSQL test database unavailable: %v (set DURABLEGO_TEST_DATABASE_URL or ensure Docker is running)", err)
		}
